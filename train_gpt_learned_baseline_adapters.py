...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,loop_gate,loop_norms",
    ).split(",")
    if pattern
)
...
class LearnedRMSNorm(nn.Module):
    ...
    Each loop through shared blocks gets its own LearnedRMSNorm,
    allowing the same weights to see differently-scaled features.
    """
    def __init__(self, dim: int):
        super().__init__()
...
class LearnedResidualAdapter(nn.Module):
    def __init__(self, dim: int, rank: int, scale: float = 1.0):
        super().__init__()
        self.down = nn.Linear(dim, rank, bias=False)
        self.up = nn.Linear(rank, dim, bias=False)
        self.scale = nn.Parameter(torch.tensor(scale, dtype=torch.float32))
        nn.init.normal_(self.down.weight, mean=0.0, std=1.0 / math.sqrt(dim))
        nn.init.zeros_(self.up.weight)

    def forward(self, x: Tensor) -> Tensor:
        return self.up(self.down(x)) * self.scale.to(dtype=x.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_dim: int = 128,
        ve_layers: str = "9,10",
        # NEW: depth recurrence params
        num_unique_layers: int = 0,  # 0 = disabled (use num_layers unique blocks)
        num_loops: int = 1,
        adapter_rank: int = 0,
        ...
        self.use_recurrence = num_unique_layers > 0 and num_loops > 1
        ...
        # Create only num_unique_layers blocks (shared across loops if recurrence enabled)
        actual_blocks = self.num_unique_layers
        ...
        # ---- PER-LOOP LEARNED NORMS (only when recurrence is active) ----
        if self.use_recurrence:
            # Each loop gets its own learned normalization per block
            # This is what differentiates each pass through the shared weights
            self.loop_norms = nn.ModuleList([
                nn.ModuleList([LearnedRMSNorm(model_dim) for _ in range(self.num_unique_layers)])
                for _ in range(self.num_loops)
            ])
            # Per-loop gate: controls how much each loop contributes
            # Loop 0 = 1.0, later loops start smaller
            gate_init = torch.ones(self.num_loops, dtype=torch.float32)
            for i in range(1, self.num_loops):
                gate_init[i] = max(1.0 - 0.3 * i, 0.3)
        ...
        self.total_effective_layers = num_layers if not self.use_recurrence else self.num_unique_layers * self.num_loops
        ...
        # XSA: apply to last N *effective* layers
        if xsa_last_n > 0:
            if self.use_recurrence:
                # For recurrence, enable XSA on the unique blocks that will be
                # used in the last xsa_last_n effective positions
                # The last loop's last blocks are the "deepest" effective layers
                for i in range(max(0, self.num_unique_layers - xsa_last_n), self.num_unique_layers):
    ...
    def _forward_blocks(self, x: Tensor, x0: Tensor, input_ids: Tensor) -> Tensor:
        """Forward pass through all blocks, with or without depth recurrence."""
        skips: list[Tensor] = []
        ve_cache: dict = {}
        effective_layer = 0
 
        if not self.use_recurrence:
            # Original path: each block is unique
            for i in range(self.num_encoder_layers):
                ve = self._get_ve(i, input_ids, ve_cache)
                x = self.blocks[i](x, x0, v_embed=ve)
                adapter = self._adapter_for_layer(effective_layer)
            ...
            # Depth recurrence path: loop through shared blocks
            for loop_idx in range(self.num_loops):
                ...
                for block_idx in range(self.num_unique_layers):
                    ...
                    x_normed = self.loop_norms[loop_idx][block_idx](x)
 
                    # Map effective layer index to VE
                    ve = self._get_ve(effective_layer, input_ids, ve_cache)
 
                    if effective_layer < self.num_encoder_layers:
                        # Encoder half: run block, gate, save skip
                        block_out = self.blocks[block_idx](x_normed, x0, v_embed=ve)
                        x = gate * block_out + (1.0 - gate) * x
                        skips.append(x)
                    else:
                        # Decoder half: add skip, run block, gate
                        decoder_idx = effective_layer - self.num_encoder_layers
                        if skips:
                            x_normed = x_normed + self.skip_weights[decoder_idx].to(dtype=x.dtype)[None, None, :] * skips.pop()
                        block_out = self.blocks[block_idx](x_normed, x0, v_embed=ve)
                        x = gate * block_out + (1.0 - gate) * x
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self._forward_blocks(x, x0, input_ids)
...
def main() -> None:
    ...
    # Depth recurrence params
    if hasattr(base_model, 'loop_norms') and len(base_model.loop_norms) > 0:
        for ln_list in base_model.loop_norms:
            for ln in ln_list:
