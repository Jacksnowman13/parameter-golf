...
class Hyperparameters:
    ...
    recurrent_enabled = bool(int(os.environ.get("RECURRENT_ENABLED", "0")))
    recurrent_shared_layers = int(os.environ.get("RECURRENT_SHARED_LAYERS", 6))
    recurrent_loops = int(os.environ.get("RECURRENT_LOOPS", 2))
    recurrent_loop_norm = bool(int(os.environ.get("RECURRENT_LOOP_NORM", "1")))
    recurrent_gated_residual = bool(int(os.environ.get("RECURRENT_GATED_RESIDUAL", "1")))
...
class LearnedRMSNorm(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return y * self.weight.to(dtype=x.dtype)[None, None, :]
...
class GPT(nn.Module):
    def __init__(
        ...
        self.recurrent_shared_layers = recurrent_shared_layers
        self.recurrent_loops = recurrent_loops
        self.recurrent_loop_norm = recurrent_loop_norm
        self.recurrent_gated_residual = recurrent_gated_residual
        self.num_layers = num_layers
        self.total_layers = recurrent_shared_layers * recurrent_loops if recurrent_enabled else num_layers
        ...
        bank_layers = recurrent_shared_layers if recurrent_enabled else num_layers
        self.bank_layers = bank_layers
        self.qo_bank = nn.Parameter(torch.empty(2 * bank_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * bank_layers, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(bank_layers, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(bank_layers, model_dim, mlp_dim))
        self.blocks = nn.ModuleList(
            [
                ...
                for i in range(bank_layers)
            ]
        )
        if recurrent_enabled:
            if recurrent_loop_norm:
                self.loop_attn_norms = nn.ModuleList([LearnedRMSNorm(model_dim) for _ in range(recurrent_loops)])
                self.loop_mlp_norms = nn.ModuleList([LearnedRMSNorm(model_dim) for _ in range(recurrent_loops)])
            else:
                self.loop_attn_norms = nn.ModuleList([nn.Identity() for _ in range(recurrent_loops)])
                self.loop_mlp_norms = nn.ModuleList([nn.Identity() for _ in range(recurrent_loops)])
            if recurrent_gated_residual:
                self.loop_attn_gates = nn.Parameter(torch.zeros(recurrent_loops, model_dim, dtype=torch.float32))
                self.loop_mlp_gates = nn.Parameter(torch.zeros(recurrent_loops, model_dim, dtype=torch.float32))
            else:
                self.loop_attn_gates = None
                self.loop_mlp_gates = None
        else:
        ...
        if xsa_last_n > 0:
            for i in range(max(0, bank_layers - xsa_last_n), bank_layers):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        if self.recurrent_enabled:
            ...
            for loop_idx in range(self.recurrent_loops):
                for layer_idx in range(self.bank_layers):
                    h = x
                    attn_in = self.loop_attn_norms[loop_idx](h) if self.recurrent_loop_norm else h
                    mlp_in = self.loop_mlp_norms[loop_idx](h) if self.recurrent_loop_norm else h
                    ve = self._get_ve(loop_idx * self.bank_layers + layer_idx, input_ids, ve_cache)
                    block = self.blocks[layer_idx]
                    attn_out, raw_v = block.attn(
                        block.attn_norm(attn_in) * block.ln_scale_factor,
                        self.qo_bank[layer_idx], self.kv_bank[layer_idx], self.kv_bank[self.bank_layers + layer_idx],
                        self.qo_bank[self.bank_layers + layer_idx], v_embed=ve, v0=v0,
                    )
                    if v0 is None and raw_v is not None:
                        v0 = raw_v
                    if self.loop_attn_gates is not None:
                        g_attn = torch.sigmoid(self.loop_attn_gates[loop_idx]).to(dtype=h.dtype)[None, None, :]
                        x = h + g_attn * (block.attn_scale.to(dtype=h.dtype)[None, None, :] * attn_out)
                    else:
                        x = h + block.attn_scale.to(dtype=h.dtype)[None, None, :] * attn_out
                    mlp_out = block.mlp(block.mlp_norm(mlp_in) * block.ln_scale_factor, self.mlp_up_bank[layer_idx], self.mlp_down_bank[layer_idx])
                    if self.loop_mlp_gates is not None:
                        g_mlp = torch.sigmoid(self.loop_mlp_gates[loop_idx]).to(dtype=x.dtype)[None, None, :]
                        x = x + g_mlp * (block.mlp_scale.to(dtype=x.dtype)[None, None, :] * mlp_out)
                    else:
                        x = x + block.mlp_scale.to(dtype=x.dtype)[None, None, :] * mlp_out
                    if block.dtg_gate is not None:
                        gate = torch.sigmoid(block.dtg_gate(h.detach()))
                        x = h + gate * (x - h)
        else:
            n = self.bank_layers
            x = self.tok_emb(input_ids)
...
def _legacy_rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        recurrent_shared_layers=args.recurrent_shared_layers,
        recurrent_loops=args.recurrent_loops,
        recurrent_loop_norm=args.recurrent_loop_norm,
        recurrent_gated_residual=args.recurrent_gated_residual,
    ).to(device).bfloat16()
    ...
    log0(f"recurrence:enabled={args.recurrent_enabled} shared_layers={args.recurrent_shared_layers} loops={args.recurrent_loops} loop_norm={args.recurrent_loop_norm} gated_residual={args.recurrent_gated_residual}")
