#!/usr/bin/env python3
"""
Parameter Golf v3 — Iterative Refinement via Depth Recurrence
==============================================================
Creative thesis: Shared transformer blocks with per-loop normalization
act as an iterative refinement process. Each pass through the same weights
sees a differently-normalized input, learning to progressively refine
representations — similar to how diffusion models iteratively denoise.

Architecture:
- 6 unique blocks × 3 loops = 18 effective layers
- 3x MLP expansion (1536 hidden)
- Per-loop per-block RMSNorm (the key differentiator for each pass)
- Per-loop residual gates (learned scalars that control each loop's contribution)
- Int6 QAT via straight-through estimator
- Muon optimizer with weight decay
- U-Net skip connections over effective layer indices

Parameter budget:
  6 unique blocks × (~2.36M per block) = ~14.2M params
  + embedding (1024×512 tied) = 524K
  + per-loop norms: 3 loops × 6 blocks × 512 dim × 2 (scale+shift) = 18K  
  + loop gates: 3 × 6 = 18 scalars
  + skip weights: 9 × 512 = 4.6K
  Total: ~14.7M params
  Int8+zlib estimate: ~10-12MB (well under 16MB)
...
class Hyperparameters:
    ...
    num_unique_layers: int = int(os.environ.get("NUM_UNIQUE_LAYERS", 6))
    ...
    use_qat: bool = bool(int(os.environ.get("USE_QAT", "1")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    p for p in os.environ.get(
        ...
        "attn_scale,mlp_scale,resid_mix,q_gain,skip_weight,skip_weights,loop_gate,loop_norms",
    ).split(",") if p
)
...
def quantize_ste_int6(x: mx.array) -> mx.array:
    if x.ndim == 2:
        abs_max = mx.maximum(mx.max(mx.abs(x), axis=1, keepdims=True), 1e-8)
        scale = abs_max / 31.0
        return mx.clip(mx.round(x / scale), -31, 31) * scale
    else:
        abs_max = mx.maximum(mx.max(mx.abs(x)), 1e-8)
        scale = abs_max / 31.0
        return mx.clip(mx.round(x / scale), -31, 31) * scale
...
# ==============================================================================
# MODEL — Iterative Refinement Architecture
# ==============================================================================

class CastedLinear(nn.Module):
    def __init__(self, in_dim, out_dim, use_qat=False):
        super().__init__()
    ...
    def __call__(self, x):
        ...
        if self.use_qat:
            w = mx.stop_gradient(quantize_ste_int6(w) - w) + w
        return x @ w.T
...
class LearnedRMSNorm(nn.Module):
    """RMSNorm with a learned scale parameter.
    This is the key component that differentiates each loop's pass
    through the shared weights — each loop sees differently-scaled features.
    """
    ...
    def __call__(self, x):
        normed = rms_norm(x)
        return normed * self.scale.astype(x.dtype)[None, None, :]


class CausalSelfAttention(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, rope_base, qk_gain_init, use_qat=False):
        super().__init__()
        self.num_heads, self.num_kv_heads = num_heads, num_kv_heads
        self.head_dim = dim // num_heads
        kv_dim = num_kv_heads * self.head_dim
        self.c_q = CastedLinear(dim, dim, use_qat=use_qat)
        self.c_k = CastedLinear(dim, kv_dim, use_qat=use_qat)
        self.c_v = CastedLinear(dim, kv_dim, use_qat=use_qat)
        self.proj = CastedLinear(dim, dim, use_qat=use_qat)
...
class MLP(nn.Module):
    def __init__(self, dim, mlp_mult, use_qat=False):
        super().__init__()
        self.fc = CastedLinear(dim, dim * mlp_mult, use_qat=use_qat)
        self.proj = CastedLinear(dim * mlp_mult, dim, use_qat=use_qat)
...
class Block(nn.Module):
    """Core transformer block — weights are shared across loops."""
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, use_qat=False):
        super().__init__()
...
class GPT(nn.Module):
    """
    Iterative Refinement GPT.
    
    The core idea: N unique transformer blocks are applied L times in sequence.
    Each application (loop) has its own learned normalization (LearnedRMSNorm)
    and a scalar gate controlling how much that loop contributes.
    
    Loop 0: Initial feature extraction (gate ≈ 1.0)
    Loop 1: First refinement pass (gate starts at 0.7, learned)
    Loop 2: Second refinement pass (gate starts at 0.5, learned)
    
    The per-loop norms are critical — they let the same weights process
    features at different scales/distributions on each pass, effectively
    giving each loop a different "view" of the data.
    
    U-Net skip connections span the full unrolled depth.
    """
    def __init__(self, vocab_size, num_unique_layers, num_loops, dim, num_heads,
                 num_kv_heads, mlp_mult, logit_chunk_tokens, logit_softcap,
                 rope_base, tied_embed_init_std, qk_gain_init, use_qat=False):
        super().__init__()
        ...
        num_effective = num_unique_layers * num_loops
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, use_qat=use_qat)
            for _ in range(num_unique_layers)
        ]

        # Per-loop, per-block learned normalization — this is what differentiates each pass
        # Each loop sees the features through a different learned lens
        self.loop_norms = [
            [LearnedRMSNorm(dim) for _ in range(num_unique_layers)]
            for _ in range(num_loops)
        ]

        # Per-loop scalar gates — control contribution of each refinement pass
        # Initialized: loop 0 = 1.0, loop 1 = 0.7, loop 2 = 0.5, etc.
        gate_init = [1.0 - 0.3 * i for i in range(num_loops)]
        self.loop_gates = mx.array(gate_init, dtype=mx.float32)
    ...
    def __call__(self, input_ids):
        ...
        for loop_idx in range(self.num_loops):
            gate = self.loop_gates[loop_idx].astype(x.dtype)

            for block_idx in range(self.num_unique_layers):
                # Per-loop normalization: different learned norm for each loop
                x_input = self.loop_norms[loop_idx][block_idx](x)

                if effective_layer < self.num_encoder_layers:
                    # Encoder: run block, gate output, save skip
                    block_out = self.blocks[block_idx](x_input, x0)
                    x = gate * block_out + (1.0 - gate) * x  # gated residual
                    skips.append(x)
                else:
                    # Decoder: add skip, run block, gate output
                    decoder_idx = effective_layer - self.num_encoder_layers
                    if skips:
                        x_input = x_input + self.skip_weights[decoder_idx].astype(x.dtype)[None, None, :] * skips.pop()
                    block_out = self.blocks[block_idx](x_input, x0)
                    x = gate * block_out + (1.0 - gate) * x  # gated residual
...
class SplitOptimizers:
    def __init__(self, model, args):
        ...
        self.scalar_keys = [
            k for k, p in params.items()
            if k in ("skip_weights", "loop_gates")
            or k.startswith("loop_norms.")
...
def validate_dataset_tokenizer_pair(data_path, tokenizer_path):
    ...
    if expected_name and Path(tokenizer_path).name != expected_name:
        raise ValueError(f"Tokenizer mismatch")
    exp = (entry.get("stats") or {}).get("files_train")
    if exp is not None:
        exp = int(exp)
        if actual > exp:
            raise ValueError(f"Too many train shards")
...
def main():
    ...
    model = GPT(
        vocab_size=args.vocab_size, num_unique_layers=args.num_unique_layers,
        num_loops=args.num_loops, dim=args.model_dim, num_heads=args.num_heads,
        ...
        qk_gain_init=args.qk_gain_init, use_qat=args.use_qat,
    )
    ...
    log(f"╔══════════════════════════════════════════════════════════╗")
    log(f"║  ITERATIVE REFINEMENT GPT — Parameter Golf v3          ║")
    log(f"╠══════════════════════════════════════════════════════════╣")
    log(f"║  {args.num_unique_layers} unique layers × {args.num_loops} loops = {args.num_layers} effective layers       ║")
    log(f"║  {args.mlp_mult}x MLP | QAT: {args.use_qat} | Muon WD: {args.muon_weight_decay}              ║")
    log(f"║  Per-loop learned norms + gated residuals               ║")
    log(f"║  Parameters: {n_params:,} (shared across loops)          ║")
    log(f"╚══════════════════════════════════════════════════════════╝")
    ...
    log(f"matrix_params:{len(opt.matrix_keys)} scalar_params:{len(opt.scalar_keys)}")
    ...
    if args.warmup_steps > 0:
        for ws in range(args.warmup_steps):
            ...
            mx.eval(wl, accum); mx.synchronize()
    ...
    qo, qs = quantize_state_dict_int8(flat_state)
    qb = zlib.compress(pickle.dumps(qo, protocol=pickle.HIGHEST_PROTOCOL), level=9)
    qp = out_dir / f"{args.run_id}_mlx_model.int8.ptz"
    with qp.open("wb") as f: f.write(qb)
    cb = len(code.encode("utf-8"))
    log(f"int8_zlib:{qp.stat().st_size} bytes | code:{cb} | total:{qp.stat().st_size+cb}")

    model.update(tree_unflatten(list(dequantize_state_dict_int8(pickle.loads(zlib.decompress(qp.read_bytes()))).items())))
    qt0 = time.perf_counter()
    qvl, qvb = eval_val(args, compiled_loss, val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut, log_fn=log)
    log(f"final_int8_roundtrip val_loss:{qvl:.4f} val_bpb:{qvb:.4f} eval_time:{1000*(time.perf_counter()-qt0):.0f}ms")
