#!/usr/bin/env python3
"""
Parameter Golf - Modified MLX Training Script (v2)
===================================================
Changes from baseline:
1. Depth recurrence: 5 unique layers looped 2x = 10 effective layers (saves ~50% params)
2. 3x MLP expansion (up from 2x)
3. Int6 QAT via straight-through estimator during training
4. Muon weight decay (0.04)
5. Larger effective model within 16MB budget thanks to parameter sharing

Run with same env vars as baseline, e.g.:
    RUN_ID=v2_recurrent \
    ITERATIONS=200 \
    TRAIN_BATCH_TOKENS=8192 \
    VAL_LOSS_EVERY=0 \
    VAL_BATCH_SIZE=8192 \
    python3 train_gpt_mlx_v2.py
"""
...
class Hyperparameters:
    ...
    # Model - CHANGED DEFAULTS
    vocab_size: int = int(os.environ.get("VOCAB_SIZE", 1024))
    num_unique_layers: int = int(os.environ.get("NUM_UNIQUE_LAYERS", 5))   # NEW: unique blocks
    num_loops: int = int(os.environ.get("NUM_LOOPS", 2))                    # NEW: loops through blocks
    model_dim: int = int(os.environ.get("MODEL_DIM", 512))
    num_heads: int = int(os.environ.get("NUM_HEADS", 8))
    num_kv_heads: int = int(os.environ.get("NUM_KV_HEADS", 4))
    mlp_mult: int = int(os.environ.get("MLP_MULT", 3))                     # CHANGED: was 2
    ...
    use_qat: bool = bool(int(os.environ.get("USE_QAT", "1")))              # NEW: int6 QAT
    ...
    muon_weight_decay: float = float(os.environ.get("MUON_WEIGHT_DECAY", 0.04))  # NEW
    ...
    def num_layers(self) -> int:
        """Effective layer count for skip connection math."""
...
def quantize_ste_int6(x: mx.array) -> mx.array:
    """Fake-quantize to 6-bit during forward pass.
    Gradient passes straight through (MLX handles this automatically
    since round/clip have defined gradients of 1 in straight-through style).
    """
    if x.ndim == 2:
        abs_max = mx.max(mx.abs(x), axis=1, keepdims=True)
        abs_max = mx.maximum(abs_max, 1e-8)
        scale = abs_max / 31.0
        x_q = mx.clip(mx.round(x / scale), -31, 31) * scale
    else:
...
class CastedLinear(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, use_qat: bool = False):
        super().__init__()
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.use_qat:
            # STE: quantize in forward, gradient flows through
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float, use_qat: bool = False):
        super().__init__()
        if dim % num_heads != 0:
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, use_qat: bool = False):
        super().__init__()
        hidden = dim * mlp_mult
        self.fc = CastedLinear(dim, hidden, use_qat=use_qat)
        self.proj = CastedLinear(hidden, dim, use_qat=use_qat)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int, rope_base: float, qk_gain_init: float, use_qat: bool = False):
        super().__init__()
...
class GPT(nn.Module):
    """GPT with depth recurrence.
    
    Instead of N unique layers, we create num_unique_layers blocks and loop
    through them num_loops times, giving num_unique_layers * num_loops effective
    layers. Each loop gets its own learned scalar gate (loop_scales) so the model
    can learn to use each pass differently.
    
    U-Net skip connections operate over the effective (unrolled) layer indices.
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        # Only num_unique_layers blocks - the key parameter savings!
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, use_qat=use_qat)
            for _ in range(num_unique_layers)
        ]

        # Per-loop per-block learned scales to differentiate each pass
        # Shape: [num_loops, num_unique_layers, dim]
        # Initialized so loop 0 = 1.0, subsequent loops start slightly smaller
        loop_scale_init = np.ones((num_loops, num_unique_layers, dim), dtype=np.float32)
        for loop_idx in range(1, num_loops):
            loop_scale_init[loop_idx] *= 0.5  # later loops contribute less initially
        self.loop_scales = mx.array(loop_scale_init)
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for loop_idx in range(self.num_loops):
            for block_idx in range(self.num_unique_layers):
                # Apply per-loop scaling to differentiate passes
                loop_scale = self.loop_scales[loop_idx, block_idx].astype(x.dtype)[None, None, :]

                if effective_layer < self.num_encoder_layers:
                    # Encoder half: run block and save skip
                    block_out = self.blocks[block_idx](x, x0)
                    x = loop_scale * block_out
                    skips.append(x)
                else:
                    # Decoder half: add skip connection then run block
                    decoder_idx = effective_layer - self.num_encoder_layers
                    if skips:
                        x = x + self.skip_weights[decoder_idx].astype(x.dtype)[None, None, :] * skips.pop()
                    block_out = self.blocks[block_idx](x, x0)
                    x = loop_scale * block_out
...
# ==============================================================================
# OPTIMIZERS (MUON WITH WEIGHT DECAY + ADAM)
# ==============================================================================
...
class SplitOptimizers:
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        self.matrix_keys = [
            ...
            if k.startswith("blocks.") and p.ndim == 2 and not any(pat in k for pat in CONTROL_TENSOR_NAME_PATTERNS)
        ]
        self.scalar_keys = [
            k for k, p in params.items()
            if k in ("skip_weights", "loop_scales") or (
                k.startswith("blocks.") and (p.ndim < 2 or any(pat in k for pat in CONTROL_TENSOR_NAME_PATTERNS))
            )
        ]
...
def validate_dataset_tokenizer_pair(data_path: str, tokenizer_path: str) -> tuple[str, int, int | None]:
    ...
    if expected_train_files is not None:
        ...
        if actual_train_files > expected_train_files:
            raise ValueError(f"{dataset_dir.name} has more train shards than expected")
...
def eval_val(args, compiled_loss, val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut, log_fn=None) -> tuple[float, float]:
...
def main() -> None:
    ...
    n_effective = args.num_layers
    log(f"run_id:{args.run_id}")
    log(f"=== DEPTH RECURRENCE: {args.num_unique_layers} unique layers x {args.num_loops} loops = {n_effective} effective layers ===")
    log(f"=== MLP expansion: {args.mlp_mult}x | QAT: {args.use_qat} | Muon WD: {args.muon_weight_decay} ===")
    log(f"model_params:{n_params} (unique, shared across loops)")
    ...
    log(f"optimizer:muon+adam muon_matrix_params:{len(opt.matrix_keys)} scalar_params:{len(opt.scalar_keys)}")
    log(f"val_bpb:enabled tokenizer_kind=sentencepiece")
    ...
    if args.warmup_steps > 0:
        ...
        mx.eval(compiled_loss(x_val, y_val))
        mx.synchronize()
        train_loader = TokenLoader(args.train_files, log_fn=log, dataset_name=dataset_name)
    ...
    log(f"total_submission_size_int8_zlib: {quant_file_bytes + code_bytes} bytes")
